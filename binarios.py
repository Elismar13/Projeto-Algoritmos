...
def converterDecimalBinario1(decimal):
    if(decimal // 2 <= 1):
        return str(decimal // 2) + str(decimal % 2)

    return str(decimal % 2) + converterDecimalBinario1(decimal//2)


#Converter de Decimal determinando numeoro minimo de bits
def converterDecimalBinario2(decimal,n = 5):
    print(decimal)
    if(decimal // 2 <= 0 and n <= 0):

        return  str(decimal // 2) + str(decimal % 2)

    return  converterDecimalBinario2(decimal//2,n -1) + str(decimal % 2)
